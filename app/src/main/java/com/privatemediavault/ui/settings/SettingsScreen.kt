...
/**
 * Settings entry point. Observes [SettingsViewModel.uiState] and renders the change-PIN
 * form, the remove-originals toggle, and the explicit lock action. One-shot
 * [SettingsEvent.NavigateToPin] events (explicit lock, Req 9.4; denied export, Req 11.2)
 * are forwarded to [onNavigateToPin] so the host shows the PIN entry screen.
 *
...
/** A single masked, numeric PIN entry field. */
@Composable
...
private fun RemoveOriginalsSection(
    ...
    Row(
        ...
        Column(modifier = Modifier.weight(1f)) {
            Text(text = "Remove originals after import", style = MaterialTheme.typography.titleMedium)
            Text(
